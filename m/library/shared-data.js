...
var note_table=$vm.module_list[m.prefix+notes].Table;
//-------------------------------------
m.set_req=function(){
    ...
    switch(field){
        ...
        case '_Notes':
            ...
            td.find('u').on('click',function(){
                $vm.load_module(m.prefix+notes,'',{task_name:m.task_name,task_uid:records[I].UID,participant_uid:records[I].Data.Participant_uid})
            })
            break;
        case '_Lock':
        ...
        $("#grid__ID td[data-id=_Notes]").each(function(index){
            if(m.records[index].sys_x!=undefined){
                ...
                $(this).find('u').on('click',function(){
                    $vm.load_module(m.prefix+notes,'',{task_name:m.task_name,task_uid:m.records[index].UID,participant_uid:m.records[index].Data.Participant_uid})
                })
            }
        });
        //--------------------------
